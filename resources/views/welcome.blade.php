  ...
  <section class="hero-gradient min-h-screen flex items-center justify-center relative">
    ...
    <div class="relative z-10 text-center text-white px-6 max-w-3xl mx-auto">
      <h1 class="text-5xl md:text-6xl font-bold mb-6">
        Gear Up for <span class="block text-transparent bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text">Greatness</span>
      </h1>
...
<span class="category-chip text-white px-3 py-1 rounded-full text-xs">
    {{ $product->kategori->nama ?? '-' }}
        ...
        <!-- Harga -->
        <span class="price-tag text-white px-4 py-2 rounded-xl font-bold text-sm inline-block mb-2">
            Rp {{ number_format($product->harga, 0, ',', '.') }}
        </span>

        <!-- Tombol Detail -->
        <a href="{{ route('pembeli.section', $product->id) }}"
